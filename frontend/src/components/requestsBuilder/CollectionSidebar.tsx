...
export default function CollectionSidebar() {
  ...
  const toggleCollection = async (collectionId: string) => {
    ...
    if (nextState && collection && !collection.isLoaded && !collection.isLoading) {
  ...
  const handleAddNewItem = async (e: React.FormEvent) => {
    ...
    if (type === "collection") {
      ...
      await addRequestAction(collectionId, folderId || null, {
        name: newItemName,
            ...
            <div key={collection.id} className="space-y-1">
              ...
              {isExpanded && (
                <div className="pl-2 ml-2 border-l border-slate-800/60 space-y-1.5">
                  {/* Folders list */}
                  {collection.folders && collection.folders.map((folder) => (
                    <FolderTreeItem
                      key={folder.id}
                      folder={folder}
                      collectionId={collection.id}
                      level={0}
                      onAddFolder={(collectionId, folderId) => setShowAddModal({ type: "folder", collectionId: collectionId, folderId: folderId })}
                      onAddRequest={(collectionId, folderId) => setShowAddModal({ type: "request", collectionId: collectionId, folderId: folderId })}
                    />
                  ))}
                  ...
                  {(!collection.folders || collection.folders.length === 0) && (!collection.requests || collection.requests.length === 0) && (
          ...
          <div className="glass-panel w-full max-w-sm rounded-xl p-5 shadow-2xl relative">
            ...
            <form onSubmit={handleAddNewItem} className="space-y-4">
              ...
              {showAddModal.type === "request" && (
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewRequestType(e.target.value as RequestItem["method"])}
                    className="block w-full bg-brand-layer-2 border border-white/5 rounded-lg text-xs text-slate-300 p-2 focus:outline-none focus:border-brand-primary"
                  >
              ...
              <div className="flex gap-3 justify-end pt-4">
                ...
                <button
                  type="submit"
                  className="px-3.5 py-1.5 bg-brand-primary hover:bg-brand-secondary text-white rounded-lg text-[11px] font-semibold cursor-pointer"
                >
                  Confirm Add
                </button>
              </div>
